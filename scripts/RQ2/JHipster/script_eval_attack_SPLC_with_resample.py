...
clf.fit(X_smp,Y_smp)
pred_init = clf.predict(X)

from sklearn.metrics import accuracy_score
acc = accuracy_score(Y,pred_init)
...
acc_attack= np.zeros([max_iter+1,1],float)
acc_attack[0,0] = acc
...
for stp in step:
    ...
    while nb_iter < max_iter:
        ...
        if label == 0:
            np.savetxt("../../../results/JHipster/RQ2/SPLC/non_acc/test_adv_attack_"+str(stp)+"_nb_displacement_"+str(max_disp)+"_label_0.csv",data.round().astype(int),delimiter=',')
        else:
            np.savetxt("../../../results/JHipster/RQ2/SPLC/acc/test_adv_attack_"+str(stp)+"_nb_displacement_"+str(max_disp)+"_label_1.csv",np.array(data).round().astype(int),delimiter=',')


        print("BEGIN RETRAINING")
        new_tr_set = np.concatenate((X_smp,data))
        label_start_attack = np.ones((test_set_size))*label
        print(Y_smp.shape)
        print(label_start_attack.shape)
        new_label_set = np.concatenate((Y_smp,label_start_attack))

        clf.fit(new_tr_set,new_label_set)

        pred_ts_set = clf.predict(X)

        acc = accuracy_score(Y,pred_ts_set)

        print("Accuracy on test set after retraining: {:.2%}".format(acc))
        acc_attack[nb_iter+1,0] = acc
    ...
    if label == 0:
        ...
        f=open("../../../results/JHipster/RQ2/SPLC/check_valid/"+out_filename,"w")
    else:
    ...
    np.savetxt("../../../results/JHipster/RQ2/SPLC/acc_adv_attack_"+str(stp)+"_nb_displacement_"+str(max_disp)+"_label_1.csv",acc_attack,delimiter=',')
